		...
		foreach ($options as $value) {
			...
			switch ( $value['type'] ) {
				...
				case 'page':
					...
					if ( $query->have_posts() ) {
						while ( $query->have_posts() ) {
							$query->the_post();
							$posts_list[get_the_ID()] = get_the_title();
						}
					}
					wp_reset_postdata();
					foreach ($posts_list as $select_ID => $option) {
						$theValue = $option;
						if (!is_numeric($select_ID))
							$theValue = $select_ID;
				...
				case 'heading':
					...
					$header_class = strtolower(strtr(str_replace(' ','',$value['name']), $iso));
					$jquery_click_hook = strtolower(strtr(str_replace(' ', '', $value['name']), $iso));
				...
				case 'backup':
					...
					$backup = sap_get_options(BACKUPS);
					$init = sap_get_options('smof_init');
